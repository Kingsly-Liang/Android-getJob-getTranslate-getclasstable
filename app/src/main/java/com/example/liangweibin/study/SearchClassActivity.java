...
public class SearchClassActivity extends AppCompatActivity {
    ...
    Intent intent;
    private WebView courseTableView;
            ...
            public void run() {
                try{
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            courseTableView = (WebView) findViewById(R.id.courseTable);
                            html = "<table border='1' cellpadding='1' cellspacing='0' width='100%' style='font-weight:800;'>"+
                                    html +
                                    "</table>";
                            WebSettings webSettings = courseTableView.getSettings();
                            //自适应屏幕
                            ...
                            courseTableView.loadData(html, "text/html; charset=UTF-8", null);
    ...
    public void login(String username, String password) throws IOException {
        ...
        Connection con1 = Jsoup.connect("http://jwxt.gdufe.edu.cn/jsxsd/xskb/xskb_list.do");  // 获取connection
        con1.header(USER_AGENT, USER_AGENT_VALUE);   // 配置模拟浏览器
        Response login1 = con1.ignoreContentType(true).followRedirects(true).method(Method.POST)
                .data(datas).cookies(rs.cookies()).execute();
